...
export default class CopyRegionalSegmentPopup extends AddRegionalSegmentPopup {
	constructor(props) {
		super(props);
		...
		var data = _.reduce(oldData, (memo, val, key) => {
			if (_.has(this.defaults, key)) {
				if (key == 'name'){
					...
					// do nothing
				} else if (key == 'cities' || key == 'postcodes') {
					memo[key] = val.join(',');
				}
				else {
					memo[key] = val;
...
CopyRegionalSegmentPopup.defaultProps = {
	isCopy: true,
	title: 'Copy Regional Segment',
	model: new Model(),
	styles: {
		display: 'block',
		position: 'fixed',
